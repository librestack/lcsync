...
const int waits = 3;
const int waits_valgrind = 5; // high for valgrind
...
static void verify_test_files(char *src, char *dst)
{
	...
	else test_log("zero length file\n");

	/* check file mode is set */
	test_assert((dsb.st_mode & 0777) == mode, "verify file mode");

	/* clean up */
	if (dmap) munmap(dmap, dsb.st_size);
	if (smap) munmap(smap, ssb.st_size);
	close(fdd);
	close(fds);
}
...
int main(void)
{
	char src[] = "0000-0073.src.tmp.XXXXXX";
	char dst[] = "0000-0073.dst.tmp.XXXXXX";
	int rc = 0;

	loginit();
	test_name("net_send_mdex() file syncing (0 length file)");
